  ...
  if [ -f "$publishReleaseYaml" ]; then
    ...
    if [ "$tagsFromYaml" != "${tags[*]}" ]; then
      ...
      if [ -n "$isCurrentCommitOnRemote" ]; then
        ...
        git add "$buildGradleFile" "$libBuildGradleFile" "$publishReleaseYaml"
...
if [ "$1" ]; then
  ...
  if [ -n "$(git tag -l "$newTag")" ]; then
    ...
    libBuildGradleFile="CircularDurationView/build.gradle.kts"
    ...
    # extract the version of the previous tag
    libPrevTagVersion=$(git show "$prevTag:$libBuildGradleFile" | grep -m 1 version | sed -e "s/$libVersionFilter/\2/" | xargs)
    libPrevTagVersionLineNumber=$(git show "$prevTag:$libBuildGradleFile" | grep -nm 1 version | grep -oe '[[:digit:]]\+:' | sed -e 's/\([[:digit:]]\+\):/\1/' | xargs)
    ...
    # extract the current version from the libBuildGradleFile
    libCurrentVersion=$(grep -m 1 version < $libBuildGradleFile | sed -e "s/$libVersionFilter/\2/" | xargs)
    libCurrentVersionLineNumber=$(grep -nm 1 version < $libBuildGradleFile | grep -oe '[[:digit:]]\+:' | sed -e 's/\([[:digit:]]\+\):/\1/' | xargs)

    # initialize match flags
    versionCodeMatch=false
    versionNameMatch=false
    libVersionMatch=false
    ...
    # validate libVersion
    if [ "$libPrevTagVersion" == "$libCurrentVersion" ]; then
      if [ $versionCodeMatch == true ] || [ $versionNameMatch == true ]; then
        echo
      fi
      echo "ERROR: current tag: $newTag has libVersion the same as the previous tag: $prevTag, please change libVersion in "$libBuildGradleFile:"$libCurrentVersionLineNumber"""
      echo "$prevTag:$libBuildGradleFile:$libPrevTagVersionLineNumber:version = $libPrevTagVersion"
      echo "$newTag:$libBuildGradleFile:$libCurrentVersionLineNumber:version = $libCurrentVersion << should be $newTag"
      libVersionMatch=true
    fi

    # check if the new tag matches the current version name
    if [ "$newTag" != "$currentVersionName" ]; then
      if [ $versionCodeMatch == true ] || [ $versionNameMatch == true ] || [ $libVersionMatch == true ]; then
        echo
      fi
    ...
    if [ "$newTag" != "$libCurrentVersion" ]; then
      echo "tag $newTag doesn't match $libBuildGradleFile:$libCurrentVersionLineNumber:version = $libCurrentVersion"
      newTagVersionNameMatch=false
    fi

    # if there are any validation errors, exit with an error code
    if [ $versionCodeMatch == true ] || [ $versionNameMatch == true ] || \
       [ $libVersionMatch == true ] || [ $newTagVersionNameMatch == false ]; then
      echo
      # offer to update the mismatching values
      read -rp "Do you want to update the versionCode/versionName in $buildGradleFile and $libBuildGradleFile? (Y/n): " choice
      ...
      case "$choice" in
        [yY]*)
          ...
          if [ $libVersionMatch == true ]; then
            newLibVersion="${newTag#v}"

            echo
            echo "updating libVersion to version = ""$newLibVersion"" ..."
            sed -i "${libCurrentVersionLineNumber}s/version.*/version = \"$newLibVersion\"/" $libBuildGradleFile
            echo "Updated libVersion to $newLibVersion in $libBuildGradleFile:$libCurrentVersionLineNumber!"
          fi
